 ...
 * monitors mouse events and adds new positions to a polyline as the user
 * identifies them. The interaction sequence for creating a line is as follows:
 * <ul>
 * <li>Arm the line builder by calling its {@link #setArmed(boolean)} method
 * with an argument of true.</li>
 * <li>Place the cursor at the first desired polyline position. Press and
 * release mouse button one.</li>
 * <li>Press button one near the next desired position, drag the mouse to the
 * exact position, then release the button. The proposed line segment will echo
 * while the mouse is dragged. Continue selecting new positions this way until
 * the polyline contains all desired positions.</li>
 * <li>Disarm the <code>LineBuilder</code> object by calling its
 * {@link #setArmed(boolean)} method with an argument of false.</li>
 * </ul>
 * <p/>
 * While the line builder is armed, pressing and immediately releasing mouse
 * button one while also pressing the control key (Ctl) removes the last
 * position from the polyline.
 * </p>
 * <p/>
 * Mouse events the line builder acts on while armed are marked as consumed.
 * These events are mouse pressed, released, clicked and dragged. These events
 * are not acted on while the line builder is not armed. The builder can be
 * continuously armed and rearmed to allow intervening maneuvering of the globe
 * while building a polyline. A user can add positions, pause entry, maneuver
 * the view, then continue entering positions.
 * </p>
 * <p/>
 * Arming and disarming the line builder does not change the contents or
 * attributes of the line builder's layer.
 * </p>
 * <p/>
 * The polyline and a layer containing it may be specified when a
 * <code>LineBuilder</code> is constructed.
 * </p>
 * <p/>
 * This class contains a <code>main</code> method implementing an example
 * program illustrating use of <code>LineBuilder</code>.
 * </p>
 * 
 * @author tag
...
public class LineBuilder extends AVListImpl {
   ...
   * drawing events from the specified world window. Either or both the polyline
   * and the layer may be null, in which case the necessary object is created.
   * 
   * @param wwd
   *          the world window to draw events from.
   * @param lineLayer
   *          the layer holding the polyline. May be null, in which case a new
   *          layer is created.
   * @param polyline
  ...
  public LineBuilder(final WorldWindow wwd, RenderableLayer lineLayer, Polyline polyline) {
    ...
    this.wwd.getModel().getLayers().addIfAbsent(this.layer);
    ...
    this.wwd.getInputHandler().addMouseMotionListener(new MouseMotionAdapter() {
      public void mouseDragged(MouseEvent mouseEvent) {
        if (armed && (mouseEvent.getModifiersEx() & MouseEvent.BUTTON1_DOWN_MASK) != 0) {
          // Don't update the polyline here because the wwd current cursor
          // position will not
          // have been updated to reflect the current mouse position. Wait to
          // update in the
          // position listener, but consume the event so the view doesn't
          // respond to it.
          if (active)
            mouseEvent.consume();
        }
   ...
   * user input and builds the polyline in response to the actions mentioned in
   * the overview above. When disarmed, the line builder ignores all user input.
   * 
   * @param armed
  ...
  // ===================== Control Panel ======================= //
  // The following code is an example program illustrating LineBuilder usage. It
  ...
  private static class LinePanel extends JPanel {
    ...
    private void makePanel(Dimension size) {
      ...
      outerPanel.setBorder(new CompoundBorder(BorderFactory.createEmptyBorder(9, 9, 9, 9),
