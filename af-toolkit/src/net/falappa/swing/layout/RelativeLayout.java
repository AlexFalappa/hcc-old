package net.falappa.swing.layout;
...
/**
 * The <code>RelativeLayout</code> class is a layout manager that lays out a container's components on the specified X or Y axis.
 * <p>
 * Components can be layed out at their preferred size or at a relative size. When relative sizing is used the component must be added to
 * the container using a relative size constraint, which is simply a Float value.
 * <p>
 * The space available for relative sized components is determined by subtracting the preferred size of the other components from the space
 * available in the container. Each component is then assigned a size based on its relative size value. For example:
 * <p>
 * container.add(component1, new Float(1)); container.add(component2, new Float(2));
 * <p>
 * There is a total of 3 relative units. If the container has 300 pixels of space available then component1 will get 100 and component2,
 * 200.
 * <p>
 * It is possible that rounding errors will occur in which case you can specify a rounding policy to use to allocate the extra pixels.
 * <p>
 * By defaults components are center aligned on the secondary axis however this can be changed at the container or component level.
 */
public class RelativeLayout implements LayoutManager2, java.io.Serializable {
    ...
    /**
     * The alignment of the Components on the other axis of the Container. For X-AXIS this would refer to the Y alignemt. For Y-AXIS this
     * would refer to the X alignment.
     */
    private float alignment = CENTER;

    /**
     * This is the gap (in pixels) which specifies the space between components It can be changed at any time and should be a non-negative
     * integer.
     */
    private int gap;

    /**
     * The gap (in pixels) used before the leading component and after the trailing component. It can be changed at any time and should be a
    ...
    /**
     * Change size of relative components to fill the space available For X-AXIS aligned components the height will be filled. For Y-AXIS
     * aligned components the width will be filled.
     */
     ...
     * <p>
     * DO_NOTHING FIRST - extra pixels added to the first relative component LAST - extra pixels added to the last relative component
     * LARGEST (default) - extra pixels added to the larger relative component EQUAL - a single pixel is added to each relative component
     ...
     * <p>
     * The preferred width of a column layout is the largest preferred width of each column in the container, plus the horizontal padding
     * times the number of columns minus one, plus the left and right insets of the target container.
     * <p>
     * The preferred height of a column layout is the largest preferred height of each row in the container, plus the vertical padding times
     ...
     * <p>
     * The minimum width of a grid layout is the largest minimum width of each column in the container, plus the horizontal padding times
     * the number of columns minus one, plus the left and right insets of the target container.
     * <p>
     * The minimum height of a column layout is the largest minimum height of each row in the container, plus the vertical padding times the
     ...
     * <p>
     * The grid layout manager determines the size of individual components by dividing the free space in the container into equal-sized
     * portions according to the number of rows and columns in the layout. The container's free space equals the container's size minus any
     * insets and any specified horizontal or vertical gap. All components in a grid layout are given the same size.
     * <p>
     * @param	target the container in which to do the layout
