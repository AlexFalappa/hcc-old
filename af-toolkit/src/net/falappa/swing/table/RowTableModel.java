...
/**
 * A TableModel that better supports the processing of rows of data. That is, the data is treated more like a row than an individual cell.
 * Hopefully this class can be used as a parent class instead of extending the AbstractTableModel when you need custom models that contain
 * row related data.
 * <p>
 * A few methods have also been added to make it easier to customize properties of the model, such as the column class and column
 * editability.
 * <p>
 * Any class that extends this class must make sure to invoke the setRowClass() and setDataAndColumnNames() methods either directly, by
 * using the various constructors, or indirectly.
 * <p>
 */
abstract class RowTableModel<T> extends AbstractTableModel {
     ...
     * <p>
     * Each column's name will be taken from the <code>columnNames</code> List and the number of colums is determined by thenumber of items
...
//
     ...
     * <p>
     * First it will check to see if a Class has been specified for the <code>column</code> by using the <code>setColumnClass</code> method.
     ...
     * <code>columnNames</code> does not have an entry for this index then the default name provided by the superclass is returned
     */
...
//
    ...
    /**
     * Moves one or more rows from the inlcusive range <code>start</code> to <code>end</code> to the <code>to</code> position in the model.
     * After the move, the row that was at index <code>start</code> will be at index <code>to</code>. This method will send a
    ...
    /**
     * Remove the specified rows from the model. Rows between the starting and ending indexes, inclusively, will be removed. Notification of
     * the rows being removed will be generated.
     * <p>
    ...
    /**
     * Remove the specified rows from the model. The row indexes in the array must be in increasing order. Notification of the rows being
     * removed will be generated.
     * <p>
